...
if __name__ == "__main__":
    ...
    aug_data = SynthradData (aug_path, include_mask=False)
    ...
    for i, (ct_fixed, mr_fixed, ct_moving, mr_moving, transform, inv_transform) in enumerate (dataloader):
        ...
        if output_transformed:
            ...
            # ct_to_mr_result_mr = vxm.layers.SpatialTransformer(fill_value=0,shift_center=False)((mr_moving.numpy(), pred_transform_1[:3,:]))
        else:
    ...
    np.save ("mr_mr_inv_results.npy", mr_mr_inv_results)
    np.save ("ct_mr_inv_results.npy", ct_mr_inv_results)
    np.save ("mr_mr_abs_results.npy", mr_mr_abs_results)
    np.save ("ct_mr_abs_results.npy", ct_mr_abs_results)
